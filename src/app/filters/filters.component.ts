...
export class FiltersComponent {
	...
	countEnabledToggles() {
		const toggleElements = document.querySelectorAll('.filters-facilities mat-slide-toggle');
		let enabledCount = 0;
		toggleElements.forEach((toggle) => {
			if (toggle.classList.contains('mat-mdc-slide-toggle-checked')) {
				enabledCount++;
			}
		});
		if (enabledCount > 0) {
	...
	countNumberOfCities = () => {
		...
		return `${this.citiesCount} ${this.citiesCount === 1 ? 'city' : 'cities'}`;
	};
